...
dbms_output.put_line('PNR - ' || v_pnr_no);
...
*/
            ELSIF SUBSTR(c1_rec.data_text,1,3) = 'K-R' THEN
                BEGIN
					v_published_fare_amt := TO_NUMBER(REGEXP_REPLACE(SUBSTR(c1_rec.data_text, INSTR(c1_rec.data_text,';',1,12)+1, INSTR(c1_rec.data_text,';',1,13)-INSTR(c1_rec.data_text,';',1,12)-1), '[A-Z]', ''));
                EXCEPTION
                WHEN OTHERS THEN 
                    core_dataw.sp_errors('AIR_TICKET','K-R',SQLCODE,REGEXP_REPLACE(SUBSTR(c1_rec.data_text, INSTR(c1_rec.data_text,';',1,12)+1, INSTR(c1_rec.data_text,';',1,13)-INSTR(c1_rec.data_text,';',1,12)-1), '[A-Z]', '')
                                               || SQLERRM);
            ...
            ELSIF SUBSTR(c1_rec.data_text,1,4) = 'KN-R' THEN
                BEGIN
                   v_selling_fare_amt := TO_NUMBER(REGEXP_REPLACE(SUBSTR(c1_rec.data_text, INSTR(c1_rec.data_text,';',1,12)+1, INSTR(c1_rec.data_text,';',1,13)-INSTR(c1_rec.data_text,';',1,12)-1), '[A-Z]', ''));
                EXCEPTION
                WHEN OTHERS THEN 
                    core_dataw.sp_errors('AIR_TICKET','KN-R',SQLCODE,REGEXP_REPLACE(SUBSTR(c1_rec.data_text, INSTR(c1_rec.data_text,';',1,12)+1, INSTR(c1_rec.data_text,';',1,13)-INSTR(c1_rec.data_text,';',1,12)-1), '[A-Z]', '')
                                               || SQLERRM);
...
dbms_output.put_line('Result of insert_ticket() ' || v_result);
                    IF v_result IS NOT NULL THEN
                        ...
                        ELSIF SUBSTR(v_result, 1, 6) = 'Error ' OR SUBSTR(v_result, 1, 6) = 'Error,' THEN
                            v_error_message := 'ERROR insert_ticket() failed, file moved to error area';
                            RAISE ticket_error;
                        END IF;
                    ELSE
