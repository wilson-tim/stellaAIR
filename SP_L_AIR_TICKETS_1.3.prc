...
emd_data_found EXCEPTION;
no_booking_ref EXCEPTION;
no_travelink_ref EXCEPTION;
...
v_commission_pct          ticket.commission_pct%TYPE;
...
v_season                  VARCHAR(3);
v_pnr_id                  ticket.pnr_id%TYPE;
v_pnr_date                DATE;
v_pnr_code                tlink_view.booking_ref%TYPE;
...
v_ins_remaining_taxes     ticket.remaining_taxes%TYPE;
...
BEGIN
  ...
  --
-- Stop processing file if it contains EMD / TMCD data (TODO add EMD / TMCD processing)
   BEGIN
      SELECT 1
      INTO   v_emd_found
      FROM   l_air 
   ...
   IF v_emd_found = 1 THEN
dbms_output.put_line('File ' || fileToProcess || ' contains EMD / TMCD data, please process manually');
        RAISE emd_data_found;
   END IF;
...
dbms_output.put_line(c1_rec.data_text);
        v_pnr_code := SUBSTR(c1_rec.data_text,7,6);
dbms_output.put_line('PNR - ' || TO_CHAR(v_pnr_code));        
...
dbms_output.put_line('Ticket Date - ' || TO_CHAR(v_ticket_issue_date,'DD-MON-YYYY'));
            ...
            WHEN OTHERS THEN
                ...
                RAISE;
            END;
dbms_output.put_line('PNR Date - ' || TO_CHAR(v_ticket_issue_date,'DD-MON-YYYY'));
...
dbms_output.put_line(c1_rec.data_text);
/* TODO  Extract EMD data for later processing here */
...
-- Actually..... TLINK_VIEW has not beem maintained since 2011 so the following IF...ELSIF...END IF is redundant
                IF v_pseudocitycode IN ('38HJ','33CI','33MV','38TW','38TI','38TS','38JT','3100','3101','31FS') AND (v_booking_ref IS NULL OR v_booking_ref = '0') THEN
                    v_tlbooking_ref := '';
                    v_result := p_stella_get_data.sp_populate_travelink_ref(v_pnr_code, v_pnr_date);
                    IF v_result IS NOT NULL THEN
                        IF SUBSTR(v_result, 1, 6) = 'Error ' THEN
                            v_error_message := v_result;
                            RAISE no_travelink_ref;
                        ELSIF v_result = '0' THEN
							core_dataw.sp_errors('AIR_TICKET','I-',SQLCODE,'File ' || fileToProcess || ' PNR ' || TO_CHAR(v_pnr_code) || ' Tkt ' || TO_CHAR(v_ticket_no) 
							    || ' - ' || v_result || ', failed to retrieve Travelink Ref: Booking Ref used: 0, Branch Code used: HAJS');
                        ELSE
                            v_tlbooking_ref := v_result;
                        END IF;
                    END IF;

                    -- Use Travelink booking ref if found
                    -- otherwise use default values
                    IF v_tlbooking_ref != '0' THEN
                        v_group       := SUBSTR(v_tlbooking_ref, 1, 1);
                        v_booking_ref := SUBSTR(v_tlbooking_ref, 2);
                    ELSE
                        -- Default group used 'L'
                        v_group := 'L';
                        -- Default booking ref used '0'
                        v_booking_ref := '0';
                    END IF;

                ELSIF v_booking_ref IS NULL OR v_booking_ref = '0' THEN
                    -- Default booking ref used '0'
                    v_booking_ref := '0';
                END IF;
/*
...
*/
                IF NOT v_void AND NOT v_exchange_found THEN
					...
					ELSE
						IF v_commission_amt = 0 AND v_commission_pct != 0 THEN
							...
							v_commission_amt := v_published_fare_amt * v_commission_pct / 100;
						ELSIF v_commission_amt != 0 AND v_commission_pct = 0 THEN
                            -- calc pct from amt / published fare
							v_commission_pct := v_commission_amt / v_published_fare_amt * 100;
						END IF;
					END IF;
                ...
                -- digits will be same as main tkt only last two digits gets changed
				...
				FOR i IN 1..v_num_tkts LOOP
					...
					v_result :=  p_stella_get_data.insert_ticket(
						...
						v_iata_num,
						v_entry_user_id,
						v_ticket_issue_date, -- CHECK FORMAT!
						v_num_pax,
						v_passenger_name,
						v_gb_tax_amt,
						v_remaining_tax_amt,
...
dbms_output.put_line('Result of insert_ticket() ' || v_result);
					IF v_result IS NOT NULL THEN
						IF SUBSTR(v_result, 1, 11) = 'Error, (fk)' THEN
							core_dataw.sp_errors('AIR_TICKET','AIR_TICKET',SQLCODE,'File ' || fileToProcess || ' PNR ' || TO_CHAR(v_pnr_code) || ' Tkt ' || TO_CHAR(v_ticket_no)
						...
						END IF;
					ELSE
						v_num_rows_inserted := v_num_rows_inserted + 1;
						IF i = 1 THEN
...
dbms_output.put_line(c1_rec.data_text);
            ...
            v_ins_selling_fare_amt   := v_published_fare_amt;
            v_ins_gb_tax_amt         := v_gb_tax_amt;
            v_ins_ub_tax_amt         := v_ub_tax_amt;
            v_ins_remaining_taxes    := v_remaining_taxes;
...
dbms_output.put_line(c1_rec.data_text);
            ...
            IF LENGTH(v_fo_record) > 10 THEN
                ...
                v_exch_ticket_nos := SUBSTR(v_fo_record, 1, v_pos2 - 1);
dbms_output.put_line('Exchange ticket numbers ' || v_exch_ticket_nos);
...
dbms_output.put_line(c1_rec.data_text);
            ...
            IF SUBSTR(c1_rec.data_text,1,2) = 'FP' AND INSTR(c1_rec.data_text, 'NONREFAGT', 1, 1) > 0 THEN
                v_pos1 := REGEXP_INSTR(c1_rec.data_text, 'NONREFAGT[A-Z]{1,3}[0-9]{1,10}', 1, 1);
                v_pos2 := INSTR(c1_rec.data_text, ';', 1, 1);
				IF v_pos2 = 0 THEN
					v_pos2 := LENGTH(c1_rec.data_text);
				END IF;
            ...
            ELSIF SUBSTR(c1_rec.data_text,1,2) = 'FP' AND INSTR(c1_rec.data_text, 'NONREF', 1, 1) > 0 THEN
                v_pos1 := REGEXP_INSTR(c1_rec.data_text, 'NONREF[A-Z]{1,3}[0-9]{1,10}', 1, 1);
                v_pos2 := INSTR(c1_rec.data_text, ';', 1, 1);
				IF v_pos2 = 0 THEN
					v_pos2 := LENGTH(c1_rec.data_text);
				END IF;
            ...
            ELSIF SUBSTR(c1_rec.data_text,1,6) = 'RM ##D' OR SUBSTR(c1_rec.data_text,1,5) = 'RM #D' THEN
                v_inside_rm := True;
                v_pos1 := REGEXP_INSTR(c1_rec.data_text, '#D[A-Z]{1,3}[0-9]{1,10}', 1, 1);
                v_pos2 := INSTR(c1_rec.data_text, ';', 1, 1);
				IF v_pos2 = 0 THEN
					v_pos2 := LENGTH(c1_rec.data_text);
				END IF;
...
dbms_output.put_line('File ' || fileToProcess || ' PNR ' || TO_CHAR(v_pnr_code) || ' Tkt ' || TO_CHAR(v_ticket_no) || ' cannot read Travelink booking ref in either FP line or RM line');
                RAISE no_booking_ref;
            END IF;
dbms_output.put_line('Group ' || v_group);
dbms_output.put_line('Booking ref ' || v_booking_ref);

        ELSIF SUBSTR(c1_rec.data_text,1,2) = 'TMCD' THEN
            -- Ticket number(s) associated with electronic miscellaneous document
dbms_output.put_line(c1_rec.data_text);
/* TODO  Processing for TMCD and EMD data */
...
EXCEPTION
  ...
  --
    ...
    WHEN emd_data_found THEN
        core_dataw.sp_errors('AIR_TICKET','AIR_TICKET',SQLCODE,'File ' || fileToProcess || ' contains EMD / TMCD data, please process manually - ' || SQLERRM);
    WHEN no_booking_ref THEN
        core_dataw.sp_errors('AIR_TICKET','AIR_TICKET',SQLCODE,'File ' || fileToProcess || ' PNR ' || TO_CHAR(v_pnr_code) || ' Tkt ' || TO_CHAR(v_ticket_no) || ' cannot read Travelink booking ref in either FP line or RM line - ' || SQLERRM);
    WHEN no_travelink_ref THEN
        core_dataw.sp_errors('AIR_TICKET','AIR_TICKET',SQLCODE,'File ' || fileToProcess || ' PNR ' || TO_CHAR(v_pnr_code) || ' Tkt ' || TO_CHAR(v_ticket_no) || ' ' || v_error_message || ' - ' || SQLERRM);
    WHEN not_inserted THEN
        core_dataw.sp_errors('AIR_TICKET','AIR_TICKET',SQLCODE,'File ' || fileToProcess || ' PNR ' || TO_CHAR(v_pnr_code) || ' Tkt ' || TO_CHAR(v_ticket_no) || ' ' || v_error_message || ' - ' || SQLERRM);
    WHEN consistency_error THEN
        core_dataw.sp_errors('AIR_TICKET','AIR_TICKET',SQLCODE,'File ' || fileToProcess || ' PNR ' || TO_CHAR(v_pnr_code) || ' Tkt ' || TO_CHAR(v_ticket_no) || ' ' || v_error_message || ' - ' || SQLERRM);
    WHEN OTHERS THEN
        core_dataw.sp_errors('AIR_TICKET','AIR_TICKET',SQLCODE,'File ' || fileToProcess || ' PNR ' || TO_CHAR(v_pnr_code) || ' Tkt ' || TO_CHAR(v_ticket_no) || ' - ' || SQLERRM);
  --    
