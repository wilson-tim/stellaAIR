...
dbms_output.put_line('EMD doc no ' || e_doc_no);
...
dbms_output.put_line('EMD ICW airline number ' || e_icw_airline_num);

                e_icw_ticket_no := SUBSTR(c1_rec.data_text, 7, v_pos2 -7);
dbms_output.put_line('EMD ICW ticket number ' || e_icw_ticket_no);
...
dbms_output.put_line('Booking ref ' || v_booking_ref);
    ...
    IF v_num_inserted_tkts != v_num_pax THEN
        v_error_message := 'ERROR num pax (' || v_num_pax || ') <> num tickets inserted (' || v_num_inserted_tkts || ')';
        RAISE ticket_error;
    END IF;
