...
-- 03/12/2019    1.4 TWilson  EMD data processing
--
...
dbms_output.put_line(c1_rec.data_text);
            ...
            IF v_pos1 > 0 AND v_pos2 > 0 AND v_pos1 < v_pos2 THEN
                e_doc_id := SUBSTR(c1_rec.data_text, v_pos1, v_pos2 - v_pos1 - 1);
                ...
                IF v_pos1 > 0 AND v_pos2 > 0 AND v_pos1 < v_pos2 THEN
                    v_temp_string := SUBSTR(c1_rec.data_text, v_pos1, v_pos2 - v_pos1 - 1);
...
*/
                ...
                FOR i IN 1..v_num_tkts LOOP
                    ...
                    IF (v_emd_found > 0 AND v_num_emd > 0) THEN
                        ...
                        INTO v_selling_fare_amt, v_remaining_tax_amt, v_airline_num, v_ticket_no, v_group, v_booking_ref
                        FROM L_AIR_EMD
                        WHERE record_no = i;
                    END IF;
...
dbms_output.put_line(c1_rec.data_text);
            v_pos1 := REGEXP_INSTR(c1_rec.data_text, 'NONREFAGT[A-Z]{1,3}[0-9]{1,10}', 1, 1);
            IF v_pos1 > 0 THEN
                v_temp_string := SUBSTR(c1_rec.data_text, v_pos1 + 9);
